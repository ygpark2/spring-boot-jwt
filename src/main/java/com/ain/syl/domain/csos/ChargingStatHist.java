...
/**
 * The persistent class for the CHARGING_STAT_HIST database table.
 * 
 */
@Entity
@Table(name="CHARGING_STAT_HIST")
@NamedQuery(name="ChargingStatHist.findAll", query="SELECT c FROM ChargingStatHist c")
public class ChargingStatHist implements Serializable {
	private static final long serialVersionUID = 1L;

	@EmbeddedId
	private ChargingStatHistPK id;
	...
	@Column(name="CURRENT_UCOST", precision=5, scale=2)
	...
	@Column(name="WAIT_CTRL_CD", length=10)
	private String waitCtrlCd;

	@Column(name="WRK_EXE_RATE", precision=5, scale=2)
	private BigDecimal wrkExeRate;

	public ChargingStatHist() {
	}

	public ChargingStatHistPK getId() {
		return this.id;
	}

	public void setId(ChargingStatHistPK id) {
		this.id = id;
	}
